...
from app.features.auth.adapter.inbound.web.dto.request import (
    CreateAPIKeyRequest,
    LoginRequest,
    RefreshTokenRequest,
    RegisterRequest,
...
from app.features.auth.adapter.inbound.web.mapper import AuthWebMapper
...
async def register(
    ...
    command = AuthWebMapper.to_register_command(body)
    result = await container.register_use_case.execute(command)
    return ApiResponse.ok(
        AuthWebMapper.to_register_response(result),
...
async def login(
    ...
    container = AuthContainer(db)
    command = AuthWebMapper.to_login_command(body)
    result = await container.login_use_case.execute(command)
    return ApiResponse.ok(AuthWebMapper.to_token_response(result))
...
async def refresh_token(
    ...
    return ApiResponse.ok(AuthWebMapper.to_access_token_response(result))
...
async def get_me(
    ...
    return ApiResponse.ok(AuthWebMapper.to_user_profile_response(user_result))
